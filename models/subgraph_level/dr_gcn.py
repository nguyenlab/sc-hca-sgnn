import torch
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.utils import scatter
from typing import Optional, Tuple

# Import encoders from graph-level
from models.graph_level.dr_gcn import DRGCNEncoder, TMPEncoder


class FunctionPooling(nn.Module):
    
    def __init__(
        self,
        hidden_dim: int,
        pooling: str = 'mean',
    ):
        super().__init__()
        self.pooling = pooling
        self.hidden_dim = hidden_dim
        
        if pooling == 'attention':
            self.attention = nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim // 2),
                nn.Tanh(),
                nn.Linear(hidden_dim // 2, 1),
            )
    
    def forward(
        self,
        x: torch.Tensor,
        subg: torch.Tensor,
        num_functions: int,
    ) -> torch.Tensor:
        
        # Only consider nodes belonging to a function (subg > 0)
        func_mask = subg > 0
        func_nodes = x[func_mask]
        func_assignment = subg[func_mask] - 1  # 0-indexed
        
        if func_nodes.size(0) == 0:
            # No function nodes, return zeros
            out_dim = self.hidden_dim * 2 if self.pooling == 'both' else self.hidden_dim
            return torch.zeros(num_functions, out_dim, device=x.device)
        
        if self.pooling == 'mean':
            return scatter(func_nodes, func_assignment, dim=0, dim_size=num_functions, reduce='mean')
        elif self.pooling == 'max':
            return scatter(func_nodes, func_assignment, dim=0, dim_size=num_functions, reduce='max')
        elif self.pooling == 'both':
            mean_pool = scatter(func_nodes, func_assignment, dim=0, dim_size=num_functions, reduce='mean')
            max_pool = scatter(func_nodes, func_assignment, dim=0, dim_size=num_functions, reduce='max')
            return torch.cat([mean_pool, max_pool], dim=-1)
        elif self.pooling == 'attention':
            # Compute attention scores
            scores = self.attention(func_nodes).squeeze(-1)  # [num_func_nodes]
            
            # Softmax within each function
            scores_exp = torch.exp(scores - scores.max())  # numerical stability
            scores_sum = scatter(scores_exp, func_assignment, dim=0, dim_size=num_functions, reduce='sum')
            scores_norm = scores_exp / (scores_sum[func_assignment] + 1e-8)
            
            # Weighted sum
            weighted = func_nodes * scores_norm.unsqueeze(-1)
            return scatter(weighted, func_assignment, dim=0, dim_size=num_functions, reduce='sum')
        else:
            raise ValueError(f"Unknown pooling method: {self.pooling}")


class FunctionLevelClassifier(nn.Module):
    
    def __init__(
        self,
        hidden_dim: int,
        num_classes: int = 2,
        dropout: float = 0.3,
    ):
        super().__init__()
        
        self.classifier = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim // 2, num_classes),
        )
    
    def forward(self, func_embeddings: torch.Tensor) -> torch.Tensor:
        return self.classifier(func_embeddings)


class FunctionLevelDRGCN(nn.Module):
    
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int = 128,
        num_layers: int = 3,
        num_classes: int = 2,
        dropout: float = 0.3,
        pooling: str = 'mean',
    ):
        super().__init__()
        
        # Full graph encoder
        self.full_encoder = DRGCNEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            dropout=dropout,
        )
        
        # Subgraph encoder (intra-function edges)
        self.sub_encoder = DRGCNEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            dropout=dropout,
        )
        
        # Function-level pooling
        pool_dim = hidden_dim * 2 if pooling == 'both' else hidden_dim
        self.pooling_layer = FunctionPooling(hidden_dim, pooling)
        
        # Classification head
        self.classifier = FunctionLevelClassifier(
            hidden_dim=pool_dim,
            num_classes=num_classes,
            dropout=dropout,
        )
        
        self.hidden_dim = hidden_dim
    
    def forward(
        self,
        x: torch.Tensor,
        edge_index: torch.Tensor,
        sub_edge_index: torch.Tensor,
        subg: torch.Tensor,
        num_functions: int,
        edge_attr: Optional[torch.Tensor] = None,
        sub_edge_attr: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        
        # Dual-path encoding
        h_full = self.full_encoder(x, edge_index)
        
        if sub_edge_index.size(1) > 0:
            h_sub = self.sub_encoder(x, sub_edge_index)
            # Combine both paths
            h = h_full + h_sub
        else:
            h = h_full
        
        # Pool by function
        func_embeddings = self.pooling_layer(h, subg, num_functions)
        
        # Classify
        logits = self.classifier(func_embeddings)
        
        return logits


class FunctionLevelTMP(nn.Module):
    
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int = 128,
        num_layers: int = 3,
        heads: int = 4,
        num_classes: int = 2,
        dropout: float = 0.3,
        pooling: str = 'mean',
    ):
        super().__init__()
        
        # Full graph encoder with temporal attention
        self.full_encoder = TMPEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            heads=heads,
            dropout=dropout,
        )
        
        # Subgraph encoder
        self.sub_encoder = TMPEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            heads=heads,
            dropout=dropout,
        )
        
        # Function-level pooling
        pool_dim = hidden_dim * 2 if pooling == 'both' else hidden_dim
        self.pooling_layer = FunctionPooling(hidden_dim, pooling)
        
        # Classification head
        self.classifier = FunctionLevelClassifier(
            hidden_dim=pool_dim,
            num_classes=num_classes,
            dropout=dropout,
        )
        
        self.hidden_dim = hidden_dim
    
    def forward(
        self,
        x: torch.Tensor,
        edge_index: torch.Tensor,
        sub_edge_index: torch.Tensor,
        subg: torch.Tensor,
        num_functions: int,
        edge_attr: Optional[torch.Tensor] = None,
        sub_edge_attr: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        
        # Extract temporal position from features (if available)
        temporal_pos = None
        if x.size(1) >= 100:
            temporal_pos = x[:, 97]  # Normalized position feature
        
        # Dual-path encoding with temporal attention
        h_full = self.full_encoder(x, edge_index, temporal_pos)
        
        if sub_edge_index.size(1) > 0:
            h_sub = self.sub_encoder(x, sub_edge_index, temporal_pos)
            h = h_full + h_sub
        else:
            h = h_full
        
        # Pool by function
        func_embeddings = self.pooling_layer(h, subg, num_functions)
        
        # Classify
        logits = self.classifier(func_embeddings)
        
        return logits


# Factory functions

def create_function_level_dr_gcn_model(
    input_dim: int,
    hidden_dim: int = 128,
    num_layers: int = 3,
    num_classes: int = 2,
    dropout: float = 0.3,
    pooling: str = 'mean',
) -> FunctionLevelDRGCN:
    
    return FunctionLevelDRGCN(
        input_dim=input_dim,
        hidden_dim=hidden_dim,
        num_layers=num_layers,
        num_classes=num_classes,
        dropout=dropout,
        pooling=pooling,
    )


def create_function_level_tmp_model(
    input_dim: int,
    hidden_dim: int = 128,
    num_layers: int = 3,
    heads: int = 4,
    num_classes: int = 2,
    dropout: float = 0.3,
    pooling: str = 'mean',
) -> FunctionLevelTMP:
    
    return FunctionLevelTMP(
        input_dim=input_dim,
        hidden_dim=hidden_dim,
        num_layers=num_layers,
        heads=heads,
        num_classes=num_classes,
        dropout=dropout,
        pooling=pooling,
    )
