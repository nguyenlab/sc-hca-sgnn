...
from models.graph_level.attention_models import GATEncoder, TransformerGNNEncoder
...
class FunctionLevelGAT(nn.Module):
    ...
    def forward(
        ...
        # Dual-path encoding with attention
...
class FunctionLevelTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Full graph encoder
        self.full_encoder = TransformerGNNEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            heads=heads,
        ...
        # Subgraph encoder
        self.sub_encoder = TransformerGNNEncoder(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            num_layers=num_layers,
            heads=heads,
    ...
    def forward(
        ...
        # Dual-path encoding with transformer attention
        h_full = self.full_encoder(x, edge_index)
...
def create_function_level_gat_model(
    input_dim: int,
    hidden_dim: int = 128,
    num_layers: int = 3,
    heads: int = 4,
...
def create_function_level_transformer_model(
    input_dim: int,
    hidden_dim: int = 128,
    num_layers: int = 3,
    heads: int = 4,
    ...
    return FunctionLevelTransformer(
        input_dim=input_dim,
        hidden_dim=hidden_dim,
        num_layers=num_layers,
        heads=heads,
